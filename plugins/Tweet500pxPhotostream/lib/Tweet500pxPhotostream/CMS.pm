package Tweet500pxPhotostream::CMS;
use strict;
...
sub _mode_tweet500pxphotostream_tweet_photo {
    my $app = shift;
    my $plugin = MT->component( 'Tweet500pxPhotostream' );
    my $blog = $app->blog;
    my $blog_id = $blog->id;
    my $photo_id = $app->param( 'photo_id' );
    my $redirect_url;
    my $log = Tweet500pxPhotostream::Util::tweet_500px_photostream_by_id( $photo_id, $blog_id );
    $redirect_url = $app->base . $app->uri( mode => 'dashboard',
                                            args => { 'tweeted-' . lc( $plugin->id ) => 1,
                                                      blog_id => $blog_id,
                                                      ( $log ? ( log_id => $log->id ) : () ),
                                                    },
                                          );
...
sub _mode_tweet500pxphotostream_oauth {
    my $app = shift;
    my $plugin = MT->component( 'Tweet500pxPhotostream' );
...
sub _mode_tweet500pxphotostream_get_access_token {
    my $app = shift;
    my $plugin = MT->component( 'Tweet500pxPhotostream' );
    my $blog = $app->blog or return $app->errtrans( 'Invalid request.' );
    my $blog_id = $blog->id;
    ...
    my $http_header = HTTP::Headers->new( 'Authorization' => $request->to_authorization_header );
    my $http_request = HTTP::Request->new( $request_method, $request_url, $http_header );
    $http_request->content( $request->to_post_body . '&oauth_verifier=' . $app->param( 'tweet500pxphotostream_pin' ) );
